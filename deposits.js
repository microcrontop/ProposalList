...
(async () => {
    ...
    const print = (targetChain) => {
        for (let key in targetChain.deposits) {
            console.log(
                "chain name", targetChain.config.name,
                "destinationChainID", targetChain.deposits[key].destinationChainID,
                "resourceID", targetChain.deposits[key].resourceID,
                "depositNonce", targetChain.deposits[key].depositNonce,
                "transactionHash", targetChain.deposits[key].transactionHash,
                "amount", targetChain.deposits[key].amount,
                "recipient", targetChain.deposits[key].recipient,
                "data", targetChain.deposits[key].data,
            );
        }
    };
